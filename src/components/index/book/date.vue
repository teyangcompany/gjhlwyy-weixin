<template>
    <div class="date">
        <div class="dateList">
            <div class="menu">
                <ul ref="wrapWidth" class="border-1px">
                    <li class="border-1px-right arrangeTime" :class="{picked:selected == index}"
                        v-for="(item,index) in timeArray" @click="selectOne(index)">
                        <p>{{timeArray[index]}}</p>
                        <p>{{ weekArray[index] }}</p>
                    </li>
                </ul>
                ...
                <div>
                    <div class="content" style="display: block">
                        <div class="border-1px" v-if="!item.bookDocId" v-for="(item,index) in commonRoom">
                            <div class="border-1px" @click="selectTime(single,index)"
                                 v-for="(single,index) in item.deptSchemeList[0].schemeList">
                                <div class="cancelImg">
                                    <img src="../../../../static/img/普通-门诊.png" alt="">
                                </div>
                                <div class="cancelIntro">
                                    <div>
                                        <span class="chatDoctor"></span> <span class="doctorTitle">{{ item.docName
                                        }}</span>
                                        <p class="oneLine">{{ item.hosName }}</p>
                                        <p style="color: #333333;">{{ single.schemeDate.substr(0, 10) }} <span
                                                style="color: #2AB6B3;">{{ single.schemeAmpm == 'am' ? '上午' : '下午'
                                            }}</span> {{ single.docName }}</p>
                                    </div>
                                </div>
                                ...
                                <div class="cancelTime" v-else>
                                    <div>
                                        ...
                                        <span class="specialMoney" v-if="single.schemeStats == 4">预约{{ single.bookFee
                                            }}元</span>
                                        <!--<img src="../../../../static/img/查看更多.png" alt="">-->
                                    </div>
                                </div>
        ...
        <time-toggle :patList="patientAll" :showPat="showPat" :option="patOption" @activate="check"
                     @toggleClosed="closeTime()"></time-toggle>
...
//          console.log(item)
                ...
                this.$router.push({
                    ...
                    query: {
                        bookDeptId: this.bookDeptId,
                        bookNumId: this.patientAll[this.clickedIndex].bookNumId,
                        numTime: this.patientAll[this.clickedIndex].numTime,
