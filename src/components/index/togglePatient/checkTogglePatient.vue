<template>
  <transition name="slide">
    <div class="wrapWhole">
      <v-header :title="title" :rightTitle="rightTitle" @0n-addUsual="addPatient()"
                @on-checkReport="goCheckReport"></v-header>
      <div class="usual" ref="usual">
        <div>
          <router-link tag="div" :to="{path:'/checkReport',query:{index:index}}" class="usualLine"
                       v-for="(item,index) in patientList" :key="item.id">
            <div class="usualCenter">
              <ul>
                <li><span class="patientName">{{ item.compatName }}</span><span>{{ item.compatAge }} &nbsp;{{ item.compatGender == "M" ? "男":"女"}}</span>
                </li>
                <li>身份证号： <span>{{ item.compatIdcard |formatCardAndMobile(3,4,11)}}</span></li>
...
<script>
  ...
  export default {
    ...
    methods: {
      addPatient() {
        this.$router.push('/checkToggleAddUsual')
      },
      goCheckReport() {
