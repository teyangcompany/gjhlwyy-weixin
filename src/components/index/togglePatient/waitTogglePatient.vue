<template>
  <transition name="slide">
    <div class="wrapWhole">
      <v-header :title="title" :rightTitle="rightTitle" @0n-addUsual="addPatient()" @on-checkReport="goCheckReport"></v-header>
      <div class="usual" ref="usual">
        <div>
          <router-link tag="div" :to="{path:'/waitCallNumber',query:{index:index}}" class="usualLine" v-for="(item,index) in patientList" :key="item.id">
...
<script>
  ...
  export default{
    ...
    methods:{
      addPatient(){
        this.$router.push('/waitToggleAddUsual')
      },
      goCheckReport(){
        this.$router.push('/waitCallNumber')
      },
      _initUsualScroll(){
