    ...
    <div class="verifyArea" v-if="allPatient != ''">
      <div class="formContent nameContent">
        <div class="form phone">
          <label for="">姓名：</label>
          <input v-if="allPatient[index].compatMedicalRecord" readonly="readonly" type="text" placeholder="请输入真实的姓名"
                 :value="changeName" v-model="changeName">
          <input v-else type="text" placeholder="请输入真实的姓名" :value="changeName" v-model="changeName">
          <span @click="doAuth(compatInfo.isRealnameAuth)">{{compatInfo.isRealnameAuth | authText}}></span>
        </div>
        <div class="form verifyCode">
          <label for="">身份证：</label>
          <input v-if="allPatient[index].compatMedicalRecord" readonly="readonly" type="text" placeholder="请输入真实的身份证号"
                 :value="changeID" v-model="changeID">
        ...
        <router-link tag="div" to="/detailUsual" class="usualLine">
          <div class="usualCenter">
            <ul>
              ...
              <li v-if="allPatient[index].compatMedicalRecord">
                病案号： <span>{{allPatient[index].compatMedicalRecord}}</span></li>
            </ul>
          </div>
        </router-link>
      </div>
...
<script>
  ...
  export default {
    ...
    computed: {
      compatInfo() {
        return this.allPatient ? this.allPatient[this.index] : {};
      }
    },
    created() {
    ...
    methods: {
      ...
      getCompatInfo() {
        api("nethos.pat.compat.list", {}).then((data) => {
          if (data.code == 0) {
      ...
      doAuth(auth) {
        ...
        !auth && this.$router.push({path, query});
      }
    },
    filters: {
      authText(auth) {
