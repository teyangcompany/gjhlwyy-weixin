<template>
  <div class="comment">
    ...
    <div class="commentArea">
      <div class="star">
        <p>评价<span>{{consultInfo.docName}}</span>医生</p>
        <div id="shoplist">
          <p class="all">
      ...
      <div class="text border-1px">
        <div>
          <textarea placeholder="请输入评论（可以不填）" onselectstart="return false;" @keyup="keypress()" id="myArea"
    ...
    methods: {
      getDetail() {
        api("nethos.consult.info.detail", {
          ...
          .then((res) => {
            if (res.code == 0) {
              this.consultInfo = res.obj.consult;
            }
          })
      },
      keypress() {
