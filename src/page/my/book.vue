      ...
      <ul>
        <li v-for="(data,index) in list" :key="index">
          <h3 class="flex">
            ...
            <div class="status flex0" :class="['status'+data.orderStatus]">{{data.orderStatus|getOrderStatus}}</div>
          </h3>
          <div class="time">
          ...
          <div class="bottom right" v-if="data.orderStatus==0||data.orderStatus==1">
            <span v-if="data.orderStatus==1" @click="handleOrder('cancel',data)">取消</span>
          </div>
        </li>
      </ul>
    </scroll>
