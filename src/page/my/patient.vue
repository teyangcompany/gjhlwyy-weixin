<template>
  <div class="page bg">
    ...
    <scroll :height="scrollHeight" class="overflow-hidden" :data="list">
      <ul>
        <li @click="onClick(index)" :key="index" tag="li" :to="{path:'/detailUsual',query:{index:index}}" class="flex"
...
<script>
  ...
  import {formatCardAndMobile, getGender, isAuth} from "../../lib/filter";
    ...
    methods: {
      onClick(index) {
        ...
        if (query && query.redirect) {
          this.$router.replace({
            path: '/' + query.redirect,
            query: Object.assign({}, query, {index})
          })
        } else {
          this.$router.push({path: '/detailUsual', query: {index: index}})
        }
      },
      async getList() {
